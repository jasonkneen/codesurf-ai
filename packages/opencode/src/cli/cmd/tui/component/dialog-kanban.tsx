...
export function DialogKanban() {
  ...
  const [layout, setLayout] = createStore({ width: 110, height: 20 })
  const applyFrame = (w: number, h: number) => dialog.setFrame?.({ width: w + 4, height: h + 12 })
  const adjustWidth = (delta: number) => {
    const next = Math.min(layoutBounds.maxWidth, Math.max(layoutBounds.minWidth, layout.width + delta))
    setLayout("width", next)
    applyFrame(next, layout.height)
  }

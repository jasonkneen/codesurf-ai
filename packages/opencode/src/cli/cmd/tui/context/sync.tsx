      ...
      session: {
        ...
        async sync(sessionID: string, options?: { force?: boolean }) {
          ...
          const [session, messages, todo, diff] = await Promise.all([
            sdk.client.session.get({ path: { id: sessionID }, throwOnError: true }).catch((error: unknown) => {
              console.error("sync session.get failed", { sessionID, error })
              throw error
            }),
            sdk.client.session.messages({ path: { id: sessionID }, query: { limit: 100 } }).catch((error: unknown) => {
              console.error("sync session.messages failed", { sessionID, error })
              throw error
            }),
            sdk.client.session.todo({ path: { id: sessionID } }).catch((error: unknown) => {
              console.error("sync session.todo failed", { sessionID, error })
              throw error
            }),
            sdk.client.session.diff({ path: { id: sessionID } }).catch((error: unknown) => {
              console.error("sync session.diff failed", { sessionID, error })
              throw error
            }),
          ])
            ...
            setStore(
              produce((draft) => {
                ...
                if (messages.data && messages.data.length >= 0) {
                  ...
                  for (const message of messages.data as any[]) {

...
export namespace Config {
  ...
  export const state = Instance.state(async () => {
    ...
    for (const file of ["opencode.jsonc", "opencode.json", "codesurf.jsonc", "codesurf.json"]) {
  ...
  export const McpLocal = z
    .object({
      ...
      disabledTools: z.string().array().optional().describe("List of tool names to disable from this MCP server"),
      timeout: z
        .number()
        .int()
        .positive()
    ...
    .object({
      ...
      anthropic: z
        .object({
          ...
          citations: z.boolean().optional().describe("Enable citations feature for source attribution (default: true)"),
          tokenEfficientToolUse: z
            .boolean()
            .optional()
            .describe("Enable token efficient tool use to reduce token consumption (default: true)"),
          fineGrainedToolStreaming: z
            .boolean()
            .optional()
            .describe("Enable fine-grained tool streaming for better progress tracking (default: true)"),
          codeExecutionTool: z
            .boolean()
            .optional()
          ...
          computerUseTool: z
            .boolean()
            .optional()
            .describe("Enable computer use tool for desktop automation (requires special setup) (default: false)"),
          textEditorTool: z
            .boolean()
            .optional()
          ...
          prefillAssistantMessages: z
            .boolean()
            .optional()
            .describe("Enable pre-fill assistant messages for guidance and role-playing (default: true)"),
          chainLongPrompts: z
            .boolean()
            .optional()
  ...
  export const global = lazy(async () => {
    let result: Info = pipe(
      ...
      mergeDeep(await loadFile(path.join(Global.Path.config, "codesurf.json"))),
      mergeDeep(await loadFile(path.join(Global.Path.config, "codesurf.jsonc"))),
    )

...
CodeSurf features an enhanced dual sidebar layout for better project organization and session management.

### Left Sidebar - Sessions

View and manage all your sessions in one place.

- **Session List**: Quick access to all active sessions
- **Visual Indicator**: Active session highlighted with ▶ marker
- **Quick Switch**: Click any session to switch instantly
- **Toggle**: `Cmd+[` or `Ctrl+[` to show/hide

### Right Sidebar - Tools & Files

Context-aware information about your current session.

**Tabs:**
- **Tools**: Recently used tools with favorites support (click ⭐ to favorite)
- **Todos**: Active todos for the current session
- **Files**: Session file changes with git status and commit interface

**Keybinds:**
- `Cmd+]` or `Ctrl+]` - Toggle right sidebar
- `Cmd+B` or `Ctrl+B` - Toggle both sidebars
- `1`, `2`, `3` - Switch between Tools, Todos, Files tabs
...
<CodeSurfBlock type="feature" title="Enhanced Session Features">

CodeSurf includes powerful session management features:

### Quick Commit Workflow

In the Files tab of the right sidebar:

1. Select files with checkboxes
2. Click **[Auto]** for automatic commit message
3. Or type custom message
4. Click **[Commit]** to commit selected files
5. Files marked with ✓ show committed status

### Session Switching

- Use left sidebar to view all sessions
- Click any session title to switch
- Active session indicated with ▶ marker
- Session context preserved across switches
...
<CodeSurfBlock type="feature" title="Favorite Tools System">

Mark frequently used tools as favorites for quick access:

### How to Favorite

Click the ★ icon next to any tool in the Tools tab to cycle through:

1. **☆ None** - Regular tool
2. **★ Project** - Favorited for this project (blue)
3. **★ Global** - Favorited across all projects (gold)

### Benefits

- Favorites appear at the top of the tools list
- Sorted by: Global → Project → Regular
- Synced via your config file
...
CodeSurf includes improved mouse support:

- Click to select text
- Click buttons and UI elements
- Scroll with mouse wheel in message area
- Mouse wheel input is filtered in the prompt area

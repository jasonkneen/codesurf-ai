...
export const { use: useLocal, provider: LocalProvider } = createSimpleContext({
  ...
  init: () => {
    ...
    function isModelValid(model: ModelKey) {
      const provider = (Array.isArray(sync.data.provider) ? sync.data.provider : []).find((x) => x.id === model.providerID)
    ...
    const agent = (() => {
      const list = createMemo(() => (Array.isArray(sync.data.agent) ? sync.data.agent : []).filter((x) => x.mode !== "subagent"))
    ...
    const model = (() => {
      ...
      const fallbackModel = createMemo(() => {
        ...
        const provider = (Array.isArray(sync.data.provider) ? sync.data.provider : [])[0]
    ...
    const file = (() => {
      const [store, setStore] = createStore<{
        ...
        node: Object.fromEntries((Array.isArray(sync.data.node) ? sync.data.node : []).map((x) => [x.path, x])),
      })

      const changeset = createMemo(() => new Set((Array.isArray(sync.data.changes) ? sync.data.changes : []).map((f) => f.path)))
